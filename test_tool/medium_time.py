...
class FactorPeriodManager:
    ...
    def _parse_yaml(self, path):
        ...
        records = []
        # 修改：读取 medium_term 节点
        for period in data.get('medium_term', []):
...
# ------------------------------------------------------------
# 股票数据加载器（与原始代码一致，仅 horizon 参数可调）
# ------------------------------------------------------------
...
def main():
    # ========== 1. 参数配置 ==========
    YAML_PATH = r"E:\tradingagents\core\stock_selection\medium_term_factors.yaml"  # 请修改为实际路径
    HS300_TOKEN = "41bc8be1587c976380a7776cb3d0e74a563aecfbfa1bef98670eb601"  # 替换为你的 token

    # 训练/测试参数
    TRAIN_WINDOW_LEN = 150      # 训练窗口长度（交易日）
    HOLDING_PERIOD = 30         # 持仓周期（交易日）
    STEP = HOLDING_PERIOD       # 滚动步长
    HORIZON = 30               # 预测超额收益周期（必须与持仓周期一致）
    ...
    for period_idx, period in enumerate(period_manager.periods):
        ...
        # 确定该区间内第一个可用的信号日索引
        # 条件：1) >= start_idx；2) >= TRAIN_WINDOW_LEN（有足够历史数据）；3) < end_idx
        first_signal_idx = start_idx
        if first_signal_idx < TRAIN_WINDOW_LEN:
            first_signal_idx = TRAIN_WINDOW_LEN
        if first_signal_idx >= end_idx:
            print(f"  区间内无足够历史数据的信号日，跳过")
            continue

        # 从 first_signal_idx 开始，步进 STEP，生成窗口
        cur_idx = first_signal_idx
        while cur_idx < end_idx:
            # 训练窗口：[cur_idx - TRAIN_WINDOW_LEN, cur_idx - 1]
            train_start_idx = cur_idx - TRAIN_WINDOW_LEN
            train_end_idx = cur_idx - 1
            # 信号日：cur_idx
            signal_idx = cur_idx
            # 持仓期：[signal_idx + 1, signal_idx + HOLDING_PERIOD]
            hold_start_idx = signal_idx + 1
            hold_end_idx = hold_start_idx + HOLDING_PERIOD - 1

            # 检查持仓期是否超出全局数据范围
            if hold_end_idx >= total_dates:
                print(f"    信号日 {all_dates[signal_idx].date()} 的持仓期超出全局数据范围，停止本区间")
                break
            ...
            print(f"    添加窗口: 信号日 {all_dates[signal_idx].date()}")
    ...
    for idx, cfg in enumerate(window_configs):
        ...
        if not factor_names:
            print(f"  警告: 信号日 {signal_date.date()} 无对应因子，跳过")
            continue
